...
scope: "contentglows remaining migration ideas"
owner: "Diane"
confidence: medium
risk_level: medium
security_impact: yes
...
linked_systems:
  ...
  - "Turso/libSQL"
evidence:
  - "contentglows/INSPIRATION.md lists video AI, audio AI, image AI, design/editor references."
  - "contentglows/SOURCE.md lists ElevenLabs, Whisper, GPT/Claude, DALL-E, Stability, Replicate, Remotion, FFmpeg, Sharp and Convex as inspiration sources."
  - "contentglows contains remotion-template, v0-flux-2-playground, v0-ai-image-generation-benchmark, v0-ai-powered-animation-studio, v0-cool-design-ressemble-gocharbon-connexion-reseaux-sociaux, v0-eleven-labs-v3-podcast-generator, v0-eleven-labs-true-crime-podcast, and v0-eleven-labs-music-starter."
...
# Exploration Report: ContentGlows Remaining Migration Ideas

## Starting Question

What remains worth migrating from `contentglows` now that the first AI visual and Remotion-related specs exist, and what additional spec candidates should be considered before implementation?

## Context Read

- `contentglows/INSPIRATION.md` - mapped the broader product inspiration across video, audio, image, design and editing.
- `contentglows/SOURCE.md` - identified provider/tool families referenced by the inspiration repo.
- `contentglows/*/package.json` - grouped prototypes by provider and capability.
- `contentglows/*/app/api/**/route.ts` - identified concrete backend behaviors in the prototypes.
- Existing `shipglows_data/workflow/specs/**` - compared already-created specs against remaining prototype ideas.
...
The prior specs covered the visual-generation spine. The remaining `contentglows` value is mostly not code-portable because the prototypes are Next/Supabase/Vercel/React, while ContentGlows uses Flutter, FastAPI, Clerk, Turso and Bunny. The useful migration unit is therefore product behavior and contracts, not source code.
...
- Pros: gives a fuller migration map from `contentglows`; avoids painting the app into an image-only architecture.
- Cons: more spec work before product proof; risks broadening scope.

## Comparison

Option B is better if the near-term goal is to audit what is still valuable in `contentglows`. Option A is better if the near-term goal is shipping the current AI visuals. The middle path is to create only the specs that affect architecture soon: audio assets, provider cost telemetry, and social placement settings.
...
- Porting `contentglows` code directly - rejected because the stack does not match and the prototypes leak implementation assumptions we do not want.
...
- User story seed: En tant que creatrice ContentGlows, je veux transformer un contenu existant en audio, podcast, musique courte ou enrichissement media guide, afin de reutiliser mes contenus sur plus de formats sans quitter le workflow editor.
...
- Why this next step: audio/podcast is the largest remaining valuable capability in `contentglows` that is not already covered by the ready visual/video specs.

## Exploration Run History

| Date UTC | Prompt/Focus | Action | Result | Next step |
|----------|--------------|--------|--------|-----------|
| 2026-05-11 16:17:00 UTC | Remaining migration ideas from `contentglows` | Read local inspiration docs, prototype directories, package manifests, route files, and current spec inventory. | Identified covered specs and five additional spec candidates. | `/sf-spec AI audio and podcast generation workflow` |
