...
evidence:
  - "Current product discussion clarified that ContentGlows should not have two competing timelines."
  - "Current product discussion clarified that ContentGlows should own the timeline model and editor experience."
...
supersedes: []
next_step: "/sf-spec Unified ContentGlows Video Timeline"
---
...
Where should ContentGlows draw the boundary between proprietary product code and rendering infrastructure for a real video timeline? Is Remotion a low-level indispensable layer, should a Dart/Flutter renderer be preferred, or should ContentGlows build the renderer itself?

## Context Read

- `shipglows_data/workflow/specs/monorepo/SPEC-remotion-video-editor-workflow-2026-05-11.md` - existing ready spec frames the current plan as a guided Remotion storyboard, not a single source-of-truth timeline.
...
The real product value is not "React vs Dart" or "Remotion vs no Remotion". The product value is a stable ContentGlows video model that users can edit, validate, version, preview and render. The renderer should be treated as an execution engine behind a boundary, not as the source of truth.

## Option Space

### Option A: ContentGlows Owns Timeline, Remotion Renders V1
...
- Pros: fastest route to reliable MP4 output, mature server-side render API, keeps ContentGlows product logic proprietary and portable.
- Cons: introduces Node/React rendering infrastructure and a Remotion dependency.

### Option B: ContentGlows Owns Timeline, Dart/Flutter Renderer Renders V1
...
### Option C: ContentGlows Builds Full Renderer From Scratch
...
ContentGlows should not own in V1:
...
- Remotion-owned product model - rejected because ContentGlows should not make Remotion props the canonical data model.
...
- User story seed: As a ContentGlows creator, I want one video timeline that can assemble images, videos, text and audio, preview it, and render final social formats without managing rendering internals.
...
- Invariants/constraints seed: ContentGlows timeline is source of truth; renderer props are derived; only server-validated asset IDs enter renders; stale preview cannot become final; renderer can be replaced behind an adapter.
...
- Recommended next command: `/sf-spec Unified ContentGlows Video Timeline`
...
|----------|--------------|--------|--------|-----------|
| 2026-05-14 14:45:50 UTC | Renderer boundary and build-vs-buy question | Compared Remotion, Dart/Flutter renderer candidates and from-scratch renderer scope | Recommended owning the timeline and adapter while using Remotion as V1 renderer behind a replaceable boundary | `/sf-spec Unified ContentGlows Video Timeline` |
| 2026-05-14 14:45:50 UTC | Maturity preference | Captured user preference for mature renderer choice | Remotion remains preferred V1 renderer; Dart/Flutter renderer stays POC/future candidate | `/sf-spec Unified ContentGlows Video Timeline` |
