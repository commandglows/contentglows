...
Verdict: keep MediaProjection. For normal capture, `MediaRecorder` is fine. For privacy capture, use `SurfaceTexture`/GPU or `ImageReader` so ContentGlows can inspect and alter frames before encoding.
