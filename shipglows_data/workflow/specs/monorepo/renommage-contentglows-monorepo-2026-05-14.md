...
owner: "Diane"
confidence: "high"
user_story: "En tant que propriétaire du produit, je veux que tout le monorepo, les surfaces publiques, les environnements et les pipelines utilisent ContentGlows/contentglows au lieu de l'ancien nom, afin que la marque, le domaine et le dépôt GitHub soient cohérents avant les prochains déploiements."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipglows_data/branding/branding.md"
    artifact_version: "1.0.0"
    required_status: "reviewed"
  - artifact: "shipglows_data/workflow/README.md"
    artifact_version: "unknown"
    required_status: "active"
supersedes: []
evidence:
  - "User request 2026-05-14: project must be contentglows / ContentGlows everywhere; domain contentglows.com has been purchased; GitHub repo target is https://github.com/diane-defores/contentglows (note: /actions is the GitHub UI page, not the git remote URL)."
  - "2026-05-15: local git remote reports https://github.com/diane-defores/contentglows.git for fetch and push."
  - "2026-05-15: active monorepo folders exist as site, app, lab, worker, and tools/design-tokens/contentglows_theme.json."
  - "2026-05-15: .github/workflows/android-apk.yml uses app and artifact contentglows-android-apk."
  - "2026-05-15: app defaults use https://contentglows.com, https://app.contentglows.com, and https://api.contentglows.com."
...
Renommage ContentGlows monorepo

## Status

Ready 2026-05-15 après gate `sf-ready` sur la reprise post-exécution partielle.

Le chantier ne repart plus de zéro. Les éléments suivants sont déjà appliqués localement et ne doivent pas être refaits aveuglément: remote Git `contentglows`, dossiers `contentglows_*`, fichier `tools/design-tokens/contentglows_theme.json`, workflow Android `app` + artefact `contentglows-android-apk`, defaults app/site/API ContentGlows, CORS/OpenAPI backend, identité Android `com.contentglows.app`, worker Remotion `ContentGlowsTimelineVideo`, et branding docs subprojets en `ContentGlows`.
...
En tant que propriétaire du produit, je veux que tout le monorepo, les surfaces publiques, les environnements et les pipelines utilisent ContentGlows/contentglows au lieu de l'ancien nom, afin que la marque, le domaine et le dépôt GitHub soient cohérents avant les prochains déploiements.

Acteur principal : Diane, propriétaire du produit et du dépôt.

Déclencheur : achat du domaine `contentglows.com` et renommage cible du dépôt GitHub vers `diane-defores/contentglows`.
...
Quand un utilisateur, un opérateur ou un pipeline interagit avec le projet après migration, le système doit présenter `ContentGlows` comme marque affichée, `contentglows` comme identifiant technique courant, `https://contentglows.com` comme site public, `https://app.contentglows.com` comme app web, `https://api.contentglows.com` comme API cible, et `https://github.com/diane-defores/contentglows` comme dépôt source. Si une dépendance externe n'est pas encore configurée, le build ou le check doit échouer clairement ou rester pilotable par variable d'environnement, sans revenir silencieusement vers l'ancien nom. L'edge case facile à rater est le mélange entre renommage visible et contrats techniques : CORS, OAuth callbacks, Android package/applicationId, artefacts CI, buckets, chemins de dossiers et clés de stockage client peuvent continuer à référencer l'ancien nom même quand la homepage est renommée.

## Success Behavior

- Depuis un clone frais du dépôt cible, `git remote -v` pointe vers `https://github.com/diane-defores/contentglows.git` ou l'équivalent SSH du même repo.
- Les dossiers actifs du monorepo sont renommés de façon cohérente : `site`, `app`, `lab`, `worker`, et `tools/design-tokens/contentglows_theme.json`, sauf si une contrainte outil impose une transition documentée.
- Les builds et checks utilisent les nouveaux chemins dans les scripts, workflows, README et outils internes.
- Les surfaces publiques affichent `ContentGlows` dans les titres, métadonnées, JSON-LD, manifestes, OG, favicon/asset metadata, blog/content authors, privacy page, app title et textes localisés.
- Les valeurs par défaut de configuration utilisent `https://contentglows.com`, `https://app.contentglows.com` et `https://api.contentglows.com`; les anciennes URLs `contentglows.winflowz.com`, `app.contentglows.winflowz.com` et `contentglows.com` ne restent que dans une note de migration ou dans une liste de redirections temporaires explicitement nommée.
- FastAPI accepte les nouvelles origines CORS, publie des métadonnées OpenAPI ContentGlows, conserve des messages d'erreur sûrs, et ne loggue aucun secret pendant la migration.
- GitHub Actions produit un artefact `contentglows-android-apk` et lit les nouveaux chemins.
...
- Si `contentglows.com`, `app.contentglows.com` ou `api.contentglows.com` ne sont pas provisionnés au moment de l'implémentation, l'agent doit arrêter avant release et documenter le blocage au lieu de livrer des defaults incohérents.
...
- Si `rg -i "contentglows|content flow"` trouve encore des occurrences actives après migration, le chantier ne passe pas ready/verify tant que chaque occurrence n'est pas soit migrée, soit classée comme historique autorisée.
...
Le renommage ContentGlows est partiellement exécuté. Les fondations visibles sont en place, mais il reste des références actives à l'ancien nom dans quelques fichiers d'app/backend/docs, et beaucoup d'occurrences dans ShipGlowz doivent être classées entre historique acceptable et source de vérité active à migrer.
...
- Validation des contrats app/backend/worker: Android applicationId, package Kotlin, manifest/provider authorities, composition ID `ContentGlowsTimelineVideo`, tests associés.
...
- Rewriting product positioning beyond the mechanical brand rename from ContentGlows to ContentGlows.
...
- Use `ContentGlows` for user-visible brand, `contentglows` for lowercase identifiers, `contentglows.com` for the apex domain, `app.contentglows.com` for app web, and `api.contentglows.com` for API unless a later operator decision overrides before implementation.
...
- Internal contract docs: subproject branding docs are reviewed at version `1.0.0` and name `ContentGlows`; this spec must preserve their tone/trust posture.
...
- Operator-owned prerequisites: DNS/domain mapping pour `contentglows.com`, `app.contentglows.com`, `api.contentglows.com`; renommage GitHub (ancien `contentglows` → `contentglows`) ; root directories Vercel et variables/secrets dans Vercel, Doppler, Clerk, Sentry, Turso, Bunny, GCP/GCS.
...
- Historical specs can mention the old name as evidence only when clearly historical; active instructions must use ContentGlows.
...
- Turso database names like `contentglows-prod2`, Bunny zones like `contentglows-images`, GCS paths like `contentglows/renders`, and Sentry releases may require explicit migration or compatibility aliases.
...
- Root `README.md`, `shipglows_data/technical/SETUP.md`, `CHANGELOG.md`.
- `shipglows_data/workflow/README.md`, subproject business/technical/editorial docs, active TASKS/AUDIT logs if they describe current state.
- `site/README.md`, `shipglows_data` docs, public content, privacy page, page intent/claim register.
...
- `ContentGlowsTimelineVideo` is now a shared backend/worker API/schema composition ID; changing it again breaks compatibility unless backend, worker, fixtures and tests change together.
- Android package paths are now under `com/contentglows/app`; any future applicationId change can affect installed app continuity and must be operator-approved if the app is published.
- App-local keys such as `contentglows:eruda` may need migration to avoid losing useful debug preferences.
- CORS regex currently allows ContentGlows deployment domains and subdomains; broad edits can over-allow domains.
- Generated comments in theme token files point to `tools/design-tokens/contentglows_theme.json`; regenerate after renaming the theme source.
...
- [ ] Tâche 1 : Vérifier l'état de reprise et protéger les changements existants
  - Fichier : `shipglows_data/workflow/specs/monorepo/renommage-contentglows-monorepo-2026-05-14.md`
  ...
  - Depends on : none.
  - Validate with : `git remote -v`, `find . -maxdepth 2 -type d -name 'contentglows_*'`, `test -f tools/design-tokens/contentglows_theme.json`.
...
- [ ] Tâche 2 : Corriger les occurrences actives résiduelles dans l'app, le site et les docs agent backend
  ...
  - Depends on : Tâche 1.
  - Validate with : `rg -n -i "contentglows|content flow" app site lab --glob '!**/.flox/**' --glob '!**/.venv_check/**'`.
  - Notes : Ne pas remplacer mécaniquement les phrases naturelles `content flows` qui ne sont pas une référence à ContentGlows.

- [ ] Tâche 3 : Vérifier et finaliser les docs root/setup/changelog
  - Fichier : `README.md`, `shipglows_data/technical/SETUP.md`, `CHANGELOG.md`
  - Action : Confirmer que clone URL, chemins, domaines, noms d'artefacts, commandes de setup et entrée de migration utilisent ContentGlows; corriger les restes actifs éventuels.
  - User story link : un opérateur suit le bon dépôt et les bons chemins.
  - Depends on : Tâche 1.
  - Validate with : `rg -n -i "contentglows|content flow|diane-defores/contentglows" README.md shipglows_data/technical/SETUP.md CHANGELOG.md`.
...
- [ ] Tâche 4 : Vérifier les scripts et tokens de design après renommage
  ...
  - Action : Confirmer que les chemins `contentglows_*` et `tools/design-tokens/contentglows_theme.json` sont utilisés, puis régénérer les tokens si une source générée est désynchronisée.
...
- [ ] Tâche 5 : Vérifier GitHub Actions Android
  ...
  - Action : Vérifier que le workflow pointe vers `app`, les nouveaux domaines et l'artefact `contentglows-android-apk`; corriger uniquement les restes si l'audit en trouve.
  - User story link : les builds CI publient des artefacts sous le nouveau nom.
  - Depends on : Tâche 1.
  - Validate with : YAML parse/check local si disponible et `rg -n -i "contentglows" .github/workflows/android-apk.yml`.
  - Notes : Variables GitHub `APP_SITE_URL`, `APP_WEB_URL`, `API_BASE_URL` restent overrideables.

- [ ] Tâche 6 : Valider le site Astro sous ContentGlows
  ...
  - User story link : le site public présente ContentGlows et les bons domaines.
  - Depends on : Tâches 2, 3.
  - Validate with : `npm run build` depuis `site` et audit `rg -n -i "contentglows|content flow" site`.
...
- [ ] Tâche 7 : Valider l'app Flutter et Android native
  ...
  - Action : Corriger les restes actifs, vérifier package name, titre, manifest, defaults URL, l10n, messages, tests, namespace/applicationId `com.contentglows.app`, Kotlin package paths et provider authorities.
  - User story link : l'expérience app affiche et configure ContentGlows.
  - Depends on : Tâches 2, 3.
...
- [ ] Tâche 8 : Valider le backend FastAPI et les contrats sécurité
  ...
  - Depends on : Tâches 2, 3.
  - Validate with : tests ciblés backend et `rg -n -i "contentglows|content flow" lab/api lab/tests lab/.env.example lab/README.md`.
...
- [ ] Tâche 10 : Valider le worker Remotion et son contrat backend
  ...
  - Action : Vérifier package/service/docs/storage path defaults et cohérence backend/worker sur `ContentGlowsTimelineVideo`.
...
- [ ] Tâche 11 : Classifier et migrer les docs de gouvernance actives
  - Fichier : `shipglows_data/business/**`, `shipglows_data/editorial/**`, `shipglows_data/technical/**`, `shipglows_data/workflow/**`
  ...
  - User story link : les agents futurs lisent ContentGlows comme source de vérité.
  - Depends on : Tâches 2-10.
  - Validate with : `rg -n -i "contentglows|content flow" shipglows_data`.
...
- [ ] Tâche 12 : Mettre à jour les tests, fixtures et snapshots de noms restants
  ...
  - Action : Adapter uniquement les assertions/fixtures qui portent encore l'ancien nom actif; vérifier les tests déjà migrés vers ContentGlows.
...
- [ ] Tâche 13 : Vérifier assets et metadata visuelles
  ...
  - User story link : les previews sociales et installations web affichent ContentGlows.
...
- [ ] Tâche 14 : Auditer les occurrences globales et classifier les restes
  ...
  - Action : Exécuter `rg -n -i "contentglows|content flow|contentglows"` avec exclusions de dossiers générés, puis migrer ou classer chaque occurrence restante.
...
- [ ] Tâche 16 : Préparer le plan opérateur de déploiement
  - Fichier : `shipglows_data/technical/SETUP.md`, docs deployment subprojects, rapport final du chantier
...
- [ ] CA 1 : Given un clone frais du dépôt, when l'opérateur lit `README.md` et `shipglows_data/technical/SETUP.md`, then il voit `ContentGlows`, `contentglows`, `diane-defores/contentglows`, et les nouveaux chemins de sous-projets.
- [ ] CA 2 : Given le remote local, when `git remote -v` est exécuté, then fetch et push pointent vers le dépôt `contentglows`.
- [ ] CA 3 : Given le workflow Android, when GitHub Actions s'exécute, then il utilise `app` et produit un artefact `contentglows-android-apk`.
- [ ] CA 4 : Given le site Astro, when `npm run build` réussit, then les canonical URLs, JSON-LD, titres, metadata et copy publique utilisent ContentGlows et `contentglows.com`.
- [ ] CA 5 : Given l'app Flutter, when l'utilisateur ouvre l'app web ou Android, then le titre, le manifest, les messages visibles et les URLs par défaut utilisent ContentGlows/contentglows.
- [ ] CA 6 : Given l'API FastAPI, when une requête vient de `https://contentglows.com` ou `https://app.contentglows.com`, then CORS l'autorise sans accepter des domaines non maîtrisés.
- [ ] CA 7 : Given l'auth web et les intégrations OAuth, when les callbacks sont documentés, then les nouvelles URLs ContentGlows sont listées et aucun secret réel n'est committé.
...
- [ ] CA 9 : Given un audit `rg -i "contentglows|content flow"`, when une occurrence reste, then elle est soit supprimée, soit classée comme historique autorisée dans le rapport de vérification.
...
- [ ] CA 12 : Given les docs ShipGlowz actives, when un agent frais lit les contextes business/techniques, then il comprend que la source de vérité est ContentGlows et non l'ancien nom.
...
- Lire d'abord cette spec, puis `README.md`, `shipglows_data/technical/SETUP.md`, `.github/workflows/android-apk.yml`, `site/src/config/site.ts`, `app/lib/core/app_config.dart`, `app/android/app/build.gradle.kts`, `lab/api/main.py`, `worker/remotion/Root.tsx`, et `worker/package.json`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-14 22:42:30 UTC | sf-spec | GPT-5 Codex | Création de la spec de migration ContentGlows après scan du monorepo, des docs, du remote Git et des occurrences de marque. | Draft saved | `/sf-ready Renommage ContentGlows monorepo` |
| 2026-05-14 22:52:42 UTC | sf-ready | GPT-5 Codex | Vérification DoR du spec de renommage ContentGlows et conformité à la doctrine de préparation avant implémentation. | Not ready | `/sf-spec Renommage ContentGlows monorepo` |
| 2026-05-14 22:55:24 UTC | sf-spec | GPT-5 Codex | Ajout explicite des dépendances externes, gates de fraîcheur doc, et garde-fous sécurité avant exécution. | Draft updated | `/sf-ready Renommage ContentGlows monorepo` |
| 2026-05-14 23:09:58 UTC | sf-ready | GPT-5 Codex | Relecture DoR du spec et évaluation finale avant /sf-start ; dépendances externes et sécurité encore non validées en docs fraîches. | Not ready | `/sf-spec Renommage ContentGlows monorepo` |
| 2026-05-14 23:26:12 UTC | sf-ready | GPT-5 Codex | Vérification DoR finale après consultation documentaire officielle pour toutes dépendances listées; mise à jour du `fresh-docs` en `checked`. | Ready | `/sf-start Renommage ContentGlows monorepo` |
| 2026-05-14 23:52:54 UTC | sf-start | GPT-5 Codex | Lancement du chantier de renommage en lot initial: validation remote, finalisation des dossiers, et migration ciblée des références actives critiques (env/site, package names, OpenAPI, docs worker). | partial | `/sf-verify Renommage ContentGlows monorepo` |
| 2026-05-15 06:17:00 UTC | sf-start | GPT-5 Codex | Tentative d'exécution depuis ce spec: blocage de contrat détecté (`status: in_progress`), donc reroutage vers gate de readiness avant nouvelle implémentation. | rerouted | `/sf-ready Renommage ContentGlows monorepo` |
| 2026-05-15 19:22:36 UTC | sf-ready | GPT-5 Codex | Revue DoR après exécution partielle: le contrat décrit encore l'état initial et ne sépare pas assez le reste à faire des tâches déjà exécutées. | Not ready | `/sf-spec Renommage ContentGlows monorepo` |
| 2026-05-15 19:39:20 UTC | sf-spec | GPT-5 Codex | Recadrage de la spec en phase de consolidation après exécution partielle: preuves actualisées, tâches déjà faites sorties du chemin critique, tâches restantes ordonnées et garde-fous sécurité conservés. | Draft updated | `/sf-ready Renommage ContentGlows monorepo` |
| 2026-05-15 19:53:23 UTC | sf-ready | GPT-5 Codex | Gate DoR stricte sur la spec recadrée: structure complète, tâches restantes actionnables, sécurité/domaines/auth bornés, et état partiel explicitement pris en compte. | Ready | `/sf-start Renommage ContentGlows monorepo` |
| 2026-05-15 20:25:00 UTC | sf-start | GPT-5 Codex | Exécution de consolidation: correction des chemins actifs restants `contentglows` dans les docs opérateur backend, maintien des expressions génériques `content flows`, et relance d'audit ciblé des occurrences actives. | implemented | `/sf-verify Renommage ContentGlows monorepo` |
| 2026-05-15 20:33:49 UTC | sf-verify | GPT-5 Codex | Vérification de l'exécution de consolidation: corrections lab confirmées, mais critères d'acceptance multi-stack et bug gate global non encore clôturés. | partial | `/sf-start Renommage ContentGlows monorepo` |
...
| 2026-05-16 06:52:41 UTC | sf-verify | GPT-5 Codex | Vérification reprise: fondations ContentGlows confirmées, mais readiness non atteinte à cause du bug gate partiel, de `lab/AGENTS.md` non symlink malgré bug clos, du check design tokens en échec, et des preuves preview/multi-stack manquantes. | partial | `/sf-start Renommage ContentGlows monorepo` |
| 2026-05-16 07:35:49 UTC | sf-build | GPT-5 Codex | Loop agents séquentiel: correction de `lab/AGENTS.md` en symlink, réduction du budget `duration` Flutter via `AppMotion.base`, et relance des validations ciblées. | partial | `/sf-test retests bug gate ContentGlows` |
